...
void screen_boilerplate(SDL_Renderer *renderer) {
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, SDL_ALPHA_OPAQUE);
    SDL_RenderClear(renderer);
}

// GBA pixel array 240x160
void plot_pixel(SDL_Renderer *renderer, Uint32 x, Uint32 y, Uint16 color) {
...
void fps_render(SDL_Renderer *renderer, Uint32 proctimestart, SDL_Surface *m_surface, TTF_Font *sans) {
...
int main(int argc, const char * argv[]) {
    ...
    while (!done) {
        ...
        screen_boilerplate(renderer);
        
        plot_pixel(renderer, mouse_x, mouse_y, 0x7FFF);
        
        
        fps_render(renderer, proctimestart, fps_surface, sans);
